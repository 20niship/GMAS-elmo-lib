...
public:
 ...
 int    LogMsgConfigureFilePathAndName(char* pcLogMsgFileName = LOG_MSG_DEF_LOG_FILE_NAME,
                ...
                /* Return 0 when everything ok and as expecting.                */
                /* If not called using the defaults.                            */
 int    LogMsgConfigureRmoveLogFiles(char* pcLogMsgFileName = LOG_MSG_DEF_LOG_FILE_NAME,                              
                                     enLOG_MSG_LOGGING_FILE_LOCATION_PATH enLogMsgLoggingFileLocation = enLOG_MSG_FLASH_FILE_PATH);
                ...
                /* Return 0 when everything ok and as expecting.                */
                /* If not called using the defaults.                            */
 int    LogMsgConfigureLogMsgSize(enLOG_MSG_SIZE_ENUM enLogMsgSize = enLOG_MSG_SIZE_SMALL);
                ...
                /* Return 0 when everything ok and as expecting.                */
                /* If not called using the defaults.                            */
 int    LogMsgConfigureMaxFileSizeB4Rotate(int iFileSizeB4RotateKB = LOG_MSG_DEF_FILE_SIZEKB);
                ...
                /* Return 0 when everything ok and as expecting.                */
                /* If not called using the defaults.                            */
 int    LogMsgConfigureNumRotateFiles(int iNumRotateFiles = LOG_MSG_DEF_ROTATE_FILES);
 ...
 int    LogMsgConfigureAndStartLoggerServer(char*                                   pcLogMsgFileName,
...
private:
 ...
 bool   LogMsgIsValidIp(char *IpAdd);
 ...
 int    LogMsgRoundFileCreation();

       /* Server Configure values */
                /* Keep hear file and path name (stay in scope along the class is) */
 char   _pcLogMsgFileAndPathStorageName[LOG_MSG_FILE_AND_PATH_STORAGE_NAME_LEN];
 ...
 int    _iLogMsgMaxFileSizeB4RotateKb;
                /* Number of rotated logs file to keep */
 int    _iLogMsgNumRotateFilesKeep;
 ...
 int    _iLogMsgServerLocalLevel;
                /* storage for remote Host Ip  */
 char   _pcLogMsgRemoteHostIp[LOG_MSG_REMOTE_IP_STORAGE_LEN];
                /* whether logging local when send logging to remote */
 bool   _bReportAlsoLocal;
                /* Remote port */
 int    _iRemoteHostPort;
