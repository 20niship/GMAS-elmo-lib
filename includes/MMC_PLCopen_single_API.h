...
typedef struct
{
	double dbPosition;					///< Target position for the motion (in technical unit [u]) (negative or positive)
	float fVelocity;					///< Value of the maximum velocity (always positive) (not necessarily reached) [u/s].
	float fAcceleration;				///< Value of the acceleration (always positive) (increasing energy of the motor) [u/s2]
	float fDeceleration;				///< Value of the deceleration (always positive) (decreasing energy of the motor) [u/s2]
	float fJerk;						///< Value of the Jerk [u/s3]. (always positive)
...
{
	double dbPosition;					///< Target position for the motion (in technical unit [u]) (negative or positive)
	double dVelocity;					///< Value of the maximum velocity (always positive) (not necessarily reached) [u/s].
	double dAcceleration;				///< Value of the acceleration (always positive) (increasing energy of the motor) [u/s2]
	double dDeceleration;				///< Value of the deceleration (always positive) (decreasing energy of the motor) [u/s2]
	double dJerk;						///< Value of the Jerk [u/s3]. (always positive)
	MC_DIRECTION_ENUM eDirection;		///< MC_Direction Enumerator type (1-of-4 values: positive_direction, shortest_way, negative_direction, current_direction)
	MC_BUFFERED_MODE_ENUM eBufferMode;	///< MC_BufferMode Defines the behavior of the axis: modes are Aborting, Buffered, Blending
	unsigned char ucExecute;			///< Start the motion at rising edge
...
typedef struct
{
	double dbDistance;					///< Relative distance for the motion (in technical unit [u])
	float fVelocity;					///< Value of the maximum velocity (always positive) (not necessarily reached) [u/s].
	float fAcceleration;				///< Value of the acceleration (always positive) (increasing energy of the motor) [u/s2]
	float fDeceleration;				///< Value of the deceleration (always positive) (decreasing energy of the motor) [u/s2]
	float fJerk;						///< Value of the Jerk [u/s3]. (always positive)
...
typedef struct
{
	unsigned char ucEnable; 	///< Get the value of the parameter continuously while enabled
...
typedef struct
{
	double dbPosition;				///<New absolute position (in axis’ unit [u])
	unsigned short usStatus;		///< Returned command status.
	short usErrorID;		///< Returned command error ID.
...
typedef struct
{
	double dVelocity;			///< The value of the actual velocity (in axis’ unit [u/s])
	unsigned short usStatus;	///< Returned command status.
	short usErrorID;	///< Returned command error ID.
...
typedef struct
{
    ...
    unsigned char ucPosMode; // abs mode = 0. relative mode = 1.
    unsigned char ucExecute;
} MMC_SETPOSITION_IN;
...
typedef struct
{
	float fDeceleration;				///< Value of the deceleration [u/s2]
	float fJerk;						///< Value of the Jerk [u/s3]
	MC_BUFFERED_MODE_ENUM eBufferMode;	///< Defines the behavior of the axis: modes are Aborting, Buffered, Blending
	unsigned char ucExecute;			///< Start the action at rising edge
}MMC_STOP_IN;
...
typedef struct
{
	unsigned int  uiHndlr;		///< Requested Axis handle.
	unsigned char ucEnable;		///< Get the value of the parameter continuously while enabled
} MMC_READSTATUS_IN;
...
typedef struct
{
	unsigned long ulState;			///< Axis state.
	unsigned short usStatus;		///< Returned command status.
	short usErrorID;		///< Returned command error ID.
	unsigned short usAxisErrorID;	///< Axis error ID.
	unsigned short usStatusWord;	///< Drive Status Word.
} MMC_READSTATUS_OUT;
...
typedef struct
{
	...
	MC_DIRECTION_ENUM eDirection;		///< MC_Direction Enumerator type (1-of-4 values: positive_direction, shortest_way, negative_direction, current_direction)
	MC_BUFFERED_MODE_ENUM eBufferMode;	///< MC_BufferMode Defines the behavior of the axis: modes are Aborting, Buffered, Blending
	unsigned int  uiExecDelayMs;		///< Defines the delay between 2 blocks (?)
	unsigned char ucExecute;			///< Start the motion at rising edge
...
typedef struct
{
	double dbDistance;					///< Target position for the motion (in technical unit [u]) (negative or positive)
	float fVelocity;					///< Value of the maximum velocity (always positive) (not necessarily reached) [u/s].
	float fAcceleration;				///< Value of the acceleration (always positive) (increasing energy of the motor) [u/s2]
	float fDeceleration;				///< Value of the deceleration (always positive) (decreasing energy of the motor) [u/s2]
	float fJerk;						///< Value of the Jerk [u/s3]. (always positive)
...
typedef struct
{
	...
	double dbTorquetVelocity;		///< Maximum Target torque velocity value
	double dbTorqueAcceleration;		///< Maximum Target torque acceleration value
...
typedef struct nc_policy_entry
{
	...
	unsigned char ucPolicy;		///< 7 bits of possible policies, MSb defines if the policies are applied to all axes
	unsigned char ucThreshold;	///< the errors threshold for starting the reaction
}
NC_POLICY_ENTRY;
...
typedef struct
{
	...
	unsigned short usAxisRef;					///< relevant only for node errors
	unsigned char ucNum;						///< num of policies to register
...
typedef struct nc_get_policy_entry
{
	unsigned char ucPolicy;		///< 7 bits of possible policies, MSb defines if the policies are applied to all axes
	unsigned char ucThreshold;	///< the errors threshold for starting the reaction
	unsigned char ucCurrentVal;	///< current value of registered err
}
NC_GET_POLICY_ENTRY;
...
typedef struct
{
	...
	unsigned short usAxisRef;				///< relevant only for node errors
	unsigned char ucNum;						///< the requested number of error policies
}MMC_GETERRPOLICY_IN;
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_MoveRelativeCmd(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_AXIS_REF_HNDL hAxisRef,
		IN MMC_MOVERELATIVE_IN* pInParam,
		OUT MMC_MOVERELATIVE_OUT* pOutParam);
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_ResetAsync(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_AXIS_REF_HNDL hAxisRef,
		IN MMC_RESET_IN* pInParam,
		OUT MMC_RESET_OUT* pOutParam);
...
/// 				 error_id in case of error
/////////////////////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_MoveAbsoluteRepetitiveCmd(IN MMC_CONNECT_HNDL hConn,
											  IN MMC_AXIS_REF_HNDL hAxisRef,
											  IN MMC_MOVEABSOLUTEREPETITIVE_IN* pInParam,
											  OUT MMC_MOVEABSOLUTEREPETITIVE_OUT* pOutParam);

MMC_LIB_API int MMC_MoveRelativeRepetitiveCmd(IN MMC_CONNECT_HNDL hConn,
								  	  	  	  IN MMC_AXIS_REF_HNDL hAxisRef,
								  	  	  	  IN MMC_MOVERELATIVEREPETITIVE_IN* pInParam,
								  	  	  	  OUT MMC_MOVERELATIVEREPETITIVE_OUT* pOutParam);

MMC_LIB_API int MMC_MoveAdditiveRepetitiveCmd(IN MMC_CONNECT_HNDL hConn,
								  	  	  	  IN MMC_AXIS_REF_HNDL hAxisRef,
								  	  	  	  IN MMC_MOVEADDITIVEREPETITIVE_IN* pInParam,
								  	  	  	  OUT MMC_MOVEADDITIVEREPETITIVE_OUT* pOutParam);

MMC_LIB_API int MMC_PositionProfileCmd(IN MMC_CONNECT_HNDL hConn,
					  	   	   	   	   IN MMC_AXIS_REF_HNDL hAxisRef,
					  	   	   	   	   IN MMC_POSITIONPROFILE_IN* pInParam,
					  	   	   	   	   OUT MMC_POSITIONPROFILE_OUT* pOutParam);

MMC_LIB_API int MMC_MoveAbsoluteExCmd(IN MMC_CONNECT_HNDL hConn,
					  	   	   	   	  IN MMC_AXIS_REF_HNDL hAxisRef,
					  	   	   	   	  IN MMC_MOVEABSOLUTEEX_IN* pInParam,
					  	   	   	   	  OUT MMC_MOVEABSOLUTEEX_OUT* pOutParam);

MMC_LIB_API int MMC_MoveRelativeExCmd(IN MMC_CONNECT_HNDL hConn,
					  	   	   	   	  IN MMC_AXIS_REF_HNDL hAxisRef,
					  	   	   	   	  IN MMC_MOVERELATIVEEX_IN* pInParam,
					  	   	   	   	  OUT MMC_MOVERELATIVEEX_OUT* pOutParam);

MMC_LIB_API int MMC_MoveAdditiveExCmd(IN MMC_CONNECT_HNDL hConn,
					  	   	   	   	  IN MMC_AXIS_REF_HNDL hAxisRef,
					  	   	   	   	  IN MMC_MOVEADDITIVEEX_IN* pInParam,
					  	   	   	   	  OUT MMC_MOVEADDITIVEEX_OUT* pOutParam);

MMC_LIB_API int MMC_MoveVelocityExCmd(IN MMC_CONNECT_HNDL hConn,
					  	   	   	   	  IN MMC_AXIS_REF_HNDL hAxisRef,
					  	   	   	   	  IN MMC_MOVEVELOCITYEX_IN* pInParam,
					  	   	   	   	  OUT MMC_MOVEVELOCITYEX_OUT* pOutParam);

MMC_LIB_API int MMC_MoveAbsoluteRepetitiveExCmd(IN MMC_CONNECT_HNDL hConn,
												IN MMC_AXIS_REF_HNDL hAxisRef,
												IN MMC_MOVEABSOLUTEREPETITIVEEX_IN* pInParam,
												OUT MMC_MOVEABSOLUTEREPETITIVEEX_OUT* pOutParam);
MMC_LIB_API int MMC_MoveRelativeRepetitiveExCmd(IN MMC_CONNECT_HNDL hConn,
												IN MMC_AXIS_REF_HNDL hAxisRef,
												IN MMC_MOVERELATIVEREPETITIVEEX_IN* pInParam,
												OUT MMC_MOVERELATIVEREPETITIVEEX_OUT* pOutParam);

MMC_LIB_API int MMC_MoveAdditiveRepetitiveExCmd(IN MMC_CONNECT_HNDL hConn,
												IN MMC_AXIS_REF_HNDL hAxisRef,
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_TouchProbeEnable(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_AXIS_REF_HNDL hAxisRef,
		IN MMC_TOUCHPROBEENABLE_IN* pInParam,
		OUT MMC_TOUCHPROBEENABLE_OUT* pOutParam) ;
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_TouchProbeDisable(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_AXIS_REF_HNDL hAxisRef,
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_RegErrPolicy(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_REGERRPOLICY_IN* pInParam,
		OUT MMC_REGERRPOLICY_OUT* pOutParam);
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_ResetSystem(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_RESETSYSTEM_IN* pInParam,
		OUT MMC_RESETSYSTEM_OUT* pOutParam);
...
/// 				 error_id in case of error
////////////////////////////////////////////////////////////////////////////////
MMC_LIB_API int MMC_GetErrPolicy(
		IN MMC_CONNECT_HNDL hConn,
		IN MMC_GETERRPOLICY_IN* pInParam,
		OUT MMC_GETERRPOLICY_OUT* pOutParam);
